...
fitBon1 <- nls(FL~vbSO(Age,Linf,K,t0,C,ts),data=bon,
               ...
               algorithm="port",control=list(maxiter=100))
## Linf=71.9, K=0.27, t0=-1.92, C=1, and to=0.09 ... they all match (within rounding)
coef(fitBon1)
################################################################################
```
...
plot(FL~Age,data=bon,pch=19,col="black",
     xlab="Age (years)",ylab="Length",xlim=c(0,4),ylim=c(0,70))
curve(VBSCGF(x,coef(fitBon2)),from=0,to=4,lwd=4,add=TRUE)
curve(vbSO(x,coef(fitBon1)),from=0,to=4,lwd=2,add=TRUE,col="gray50")
legend("bottomright",c(TeX("\\mathbf{Estimated Values}"),
                       TeX(paste0("$L_{\\infty}$"," = ",
                                  formatC(cfBon2["Linf","Est"],format="f",digits=1),
                                  " (",formatC(cfBon2["Linf","2.5%"],format="f",digits=1),
                                  ", ",formatC(cfBon2["Linf","97.5%"],format="f",digits=1),")")),
                   paste0("K' = ",formatC(cfBon2["K","Est"],format="f",digits=2),
                                  " (",formatC(cfBon2["K","2.5%"],format="f",digits=2),
                                  ", ",formatC(cfBon2["K","97.5%"],format="f",digits=2),")"),
                   TeX(paste0("$t_{0}$"," = ",
                                  formatC(cfBon2["t0","Est"],format="f",digits=2),
                                  " (",formatC(cfBon2["t0","2.5%"],format="f",digits=2),
                                  ", ",formatC(cfBon2["t0","97.5%"],format="f",digits=2),")")),
                   paste0("WP = ",formatC(cfBon2["WP","Est"],format="f",digits=2),
                                  " (",formatC(cfBon2["WP","2.5%"],format="f",digits=2),
                                  ", ",formatC(cfBon2["WP","97.5%"],format="f",digits=2),")"),
                   paste0("NGT = ",formatC(cfBon2["NGT","Est"],format="f",digits=2),
                                  " (",formatC(cfBon2["NGT","2.5%"],format="f",digits=2),
                                  ", ",formatC(cfBon2["NGT","97.5%"],format="f",digits=2),")")),
        bty="n",cex=0.8)

figcaps("PaulyBonito",paste0("Fork length at age for Australian Bonito with the best-fit of ",eqncaps('PaulyMod2',display='cite')," (black line) and ",eqncaps('SomersMod',display='cite')," (gray line) superimposed.  The parameter estimates (and 95% confidence intervals) from non-linear regression for ",eqncaps('PaulyMod2',display='cite')," are shown the lower-right corner."))
```

`r figcaps("PaulyBonito")`
